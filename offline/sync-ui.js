...
(function () {
  ...
  const HELP_TOPICS = [
    ...
    {
      q: "「固定收/支出」是做什麼的？",
      a: "用來設定會重複發生的收支，例如房租、訂閱費、薪水。設定好類型、分類、帳戶、金額跟頻率（每月幾號、或每年幾月幾號）之後，到了那一天系統會自動幫你記一筆，不用每次手動輸入。",
    },
    {
      q: "股利要怎麼記帳／查詢？",
      a: "有兩個地方都跟股利有關，功能不一樣：「股票持股」裡的「查詢股利」只是查這檔股票預期的股利資料（頻率、殖利率、除息日／發放日），單純參考用，不會自動記帳。查到資料後卡片上會多一顆「填入固定收入」，按下去會直接跳到「固定收/支出」的新增畫面，自動帶入名稱、分類（投資收入）、發放日期跟建議金額（每股股利 × 股數，再扣 10 元手續費換算出來的，僅供參考）。等錢真的匯進戶頭之後，把金額改成銀行實際入帳的數字（已經算過二代健保跟手續費）再儲存，才會真的記一筆收入。",
    },
    {
      q: "「清除本機資料」是做什麼的？",
