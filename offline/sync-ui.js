...
(function () {
  ...
  const HELP_TOPICS = [
    ...
    {
      q: "股利要怎麼記帳／查詢？",
      a: "有兩個地方都跟股利有關，功能不一樣：「股票持股」裡的「查詢股利」只是查這檔股票預期的股利資料（頻率、殖利率、除息日／發放日），單純參考用，不會自動記帳。等錢真的匯進戶頭之後，要到「固定收/支出」按「＋新增固定收支」，頻率選「股利」，填入發放日期跟實際金額，才會真的記一筆收入——這筆金額已經算過二代健保跟手續費，但還是要以銀行實際入帳金額為準。",
    },
    {
      q: "「清除本機資料」是做什麼的？",
